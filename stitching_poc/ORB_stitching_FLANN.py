...
orb = cv2.ORB_create(nfeatures) # Defining ORB variable to use the ORB algorithm to extract features of image
while True:
  if len(img_list) == 0:
    break
  img1=img_list.pop(0)
  ...
  index_params = dict(algorithm=6, # Defining index parameters for FLANN alforithm , recommended parameters for ORB
                    table_number=6,
                    key_size=12,
                    multi_probe_level=2)
  search_params = {}
  flann = cv2.FlannBasedMatcher(index_params, search_params) # Definee variable for FLANN algorithm using inputs defined previously
  matches = flann.knnMatch(des1, des2, k=2) # Use FLANN to match based on k nearest neighbours
  print(matches)
